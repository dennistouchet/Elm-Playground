...
Elm.Main.make = function (_elm) {
   ...
   var view = F2(function (address,model) {
      return A2($Html.p,_U.list([]),_U.list([$Html.text(model)]));
   });
   var update = F2(function (action,model) {
      var name = function (maybeName) {
         return A2($Maybe.withDefault,"error",maybeName);
      };
      var _p0 = action;
      return {ctor: "_Tuple2",_0: name(_p0._0),_1: $Effects.none};
   });
   var NewUser = function (a) {
      return {ctor: "NewUser",_0: a};
   };
   var userUrl = "http://jsonplaceholder.typicode.com/users/1";
   var decodeName = A2($Json$Decode._op[":="],
   "name",
   $Json$Decode.string);
   var getName = $Effects.task(A2($Task.map,
   NewUser,
   $Task.toMaybe(A2($Http.get,decodeName,userUrl))));
   var init = {ctor: "_Tuple2",_0: "...",_1: getName};
